...
object RandomRepeatHandler :
  ...
  private const val PROB_REPEAT = 0.05

  // probability of deterioration (self-repeating)
  private const val PROB_DETER = 0.5

  // probability of deterioration (follow)
  private const val PROB_DETER_FOLLOW = 0.8
  ...
  private const val NOUNS_THRESH = 4
  ...
  override suspend fun handle(event: GroupMessageEvent): Boolean {
    ...
    val texts = event.message.filterIsInstance<PlainText>().filter { it.content.trim().isNotEmpty() }
  ...
  private fun chaosString(message: String): String {
    ...
    return newWords.joinToString(separator = "") { it.first }
  }
  ...
  private fun checkLastMsg(groupId: Long, message: String): Boolean {
    return if (lastMessage[groupId]?.let { message == it } == true) {
