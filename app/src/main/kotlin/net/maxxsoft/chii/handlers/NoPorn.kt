...
object NoPornHandler :
  ...
  private suspend fun checkIn(event: GroupMessageEvent): Boolean {
    val msg =
      Database().use { db ->
        ...
        db.executeQuery("SELECT last_checkin_time, lasting_days FROM $tableName $where;") {
          if (it.next()) {
            ...
            when (val diffDays = ChronoUnit.DAYS.between(lastCheckInTime, now)) {
              0L -> Pair("今天已经打过卡了，好好戒色（摸头", false)
              1L -> {
  ...
  private suspend fun getRandom() = HttpClient().use {
    runCatching {
